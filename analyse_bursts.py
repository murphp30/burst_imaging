...
R_sun = Angle(R_sun)
if not os.path.isfile('all_bursts_modelfit.pkl'):
    pickle_list = glob.glob('burst_properties_modelfit2019*pkl')
    pickle_list.sort()
    df_list = []
    ...
    df.to_pickle('all_bursts_modelfit.pkl')
else:
    df = pd.read_pickle('all_bursts_modelfit.pkl')
...
#     mean_df = mean_df.T
df = df.T
bad_sig = 600/(2 * np.sqrt(2 * np.log(2)))
