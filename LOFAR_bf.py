...
class LOFAR_BF:
    ...
    def __get_data(self):
        ...
        with h5py.File(self.bf_file, 'r') as h5:
            ...
            if dt_unit == 's':
                fmt = 'sec'
            self.dt = TimeDelta(h5['SUB_ARRAY_POINTING_' + sap[-3:] + '/BEAM_' + beam[-3:]].attrs['SAMPLING_TIME'],
                                format=fmt)
            self.n_samples = h5['SUB_ARRAY_POINTING_' + sap[-3:] + '/BEAM_' + beam[-3:]].attrs['NOF_SAMPLES']
            self.n_channels = h5['SUB_ARRAY_POINTING_' + sap[-3:] + '/BEAM_' + beam[-3:]].attrs[
                                  'CHANNELS_PER_SUBBAND'] * h5[
                                  'SUB_ARRAY_POINTING_' + sap[-3:] + '/BEAM_' + beam[-3:] + '/STOKES_' + stokes[
                                      1]].attrs['NOF_SUBBANDS']

            self.freq_unit = u.Quantity(1, h5[
                'SUB_ARRAY_POINTING_' + sap[-3:] + '/BEAM_' + beam[-3:] + '/COORDINATES/COORDINATE_1'].attrs[
                'AXIS_UNITS'][0])
            freq_arr = h5['SUB_ARRAY_POINTING_' + sap[-3:] + '/BEAM_' + beam[-3:] + '/COORDINATES/COORDINATE_1'].attrs[
                           'AXIS_VALUES_WORLD'] * self.freq_unit
            if self.frange is None and self.trange is None:
                ...
                if self.frange is not None and self.trange is not None:
                    ...
                    f1 = np.where(abs(self.frange[1] - freq_arr) == np.min(abs(self.frange[1] - freq_arr)))[0][0]
                    self.freqs = freq_arr[f0:f1]
                    self.data = h5['SUB_ARRAY_POINTING_' + sap[-3:] + '/BEAM_' + beam[
                                    -3:] + '/STOKES_' + stokes[1]][:, f0:f1]
    ...
    def plot(self, ax=None, title=None, bg_subtract=False, scale="linear", clip_interval: u.percent = None):
        ...
        if self.trange is None:
            ret = ax.imshow(data.T, aspect="auto",
                            extent=[self.obs_start.plot_date, self.obs_end.plot_date, self.freqs.to(u.MHz).value[-1],
